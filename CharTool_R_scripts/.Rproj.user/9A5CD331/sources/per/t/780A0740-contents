...
# If libraries need to be installed, uncomment the line below:
#install,packages(c(tools, ggplot2, dplyr, moments, caret, cowplot, colorspace, scales, data.table, munsell))
...
library(colorspace) # version 1.4-1
library(scales) # version 0.5.0
library(data.table) # version 1.13.2
...
temp$Area = temp$Area 
temp$PA = temp$Perim. / temp$Area # ratio of area to perimeter
temp$PMAJ = temp$Perim. / temp$Major # ratio of perimeter to the longest axis of fragment
temp$Rect = temp$Area / ((temp$Width * temp$Height)) # rectangularity
temp$Ellipse = temp$Perim. / (2 * pi * sqrt(((temp$Major^2) + (temp$Minor))/2)) # best fit ellipse
temp$Struct = (temp$Min + temp$Max) /2 # Average BW color value (between 0 - 255)
