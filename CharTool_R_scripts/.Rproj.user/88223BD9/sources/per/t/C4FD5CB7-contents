...
# User Notes:
# Script developed for R version 3.3.2 (2016-10-31) -- "Sincere Pumpkin Patch"
...
# Inventories - Place an inventory of the samples to be analyzed in this folder. Note that the sample IDs or numbers in the inventory must match the correpsonding file names for the raw CharTool output .csv files
...
list2env( # circle through directory and load in each .csv collcted by CharTool
...
temp$Rect = temp$Perim. / ((temp$Width + temp$Height)*2)
temp$Ellipse = temp$Perim. / (2 * pi * sqrt(((temp$Major^2) + (temp$Minor))/2)) 
temp$Struct = (temp$Min + temp$Max) /2
...
# 2.4 Merge charcaol data with inventory and create a complete dataframe of all observations
...
# 2.5 add NA rows to accomadate for any gaps or missing depths 
# Note: This step is only required if gaps exist in the data collected from the core, soil probe, augar, or sediment column in the field
...
# Proceed with the remainder of the code
  ...
  dplyr::summarise(
    ...
    Area_min = min(Area),
    Area_max =  max(Area),
    Dia_avg = mean(Feret)/1000,
    Dia_min = min(Feret)/1000,
    Dia_max =  max(Feret)/1000,
