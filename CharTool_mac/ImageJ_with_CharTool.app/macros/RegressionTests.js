  ...
  function particleAnalyzerTests() {
    msg = "FAIL: ParticleAnalyzerTests #";
    ...
    IJ.run(img, "Analyze Particles...", "  show=Overlay clear");
    ...
    for (i=0; i<n; i++) {
       ...
       stats = img.getStatistics();
       //print(i+" "+stats.area+" "+IJ.d2s(stats.mean,6)+" "+img.getRoi());
       if (i==6 && stats.area!=14964) print(msg+2);
       if (i==6 && !IJ.d2s(stats.mean,6).equals("82.933975")) print(msg+3);
       if (i==119 && stats.area!=44) print(msg+4);
       if (i==119 && !IJ.d2s(stats.mean,6).equals("83.795455")) print(msg+5);
       if (stats.area!=rt.getValue("Area",i)) print(msg+6);
       if (stats.mean!=rt.getValue("Mean",i)) print(msg+7);
    }
    img.deleteRoi();
    IJ.run(img, "Analyze Particles...", " clear add");
    rt = ResultsTable.getResultsTable();
    if (rt.size()!=n) print(msg+8);
    rm = RoiManager.getInstance();
    for (i=0; i<n; i++) {
       rm.select(img, i);
       stats = img.getStatistics();
       if (stats.area!=rt.getValue("Area",i)) print(msg+9);
       if (stats.mean!=rt.getValue("Mean",i)) print(msg+10);
    }
